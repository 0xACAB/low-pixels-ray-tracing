  ...
  constructor(
    ...
    if (program && vertexShader && fragmentShader) {
      ...
      if (!context.getProgramParameter(program, context.LINK_STATUS)) {
        ...
        const texcoordLocation = context.getAttribLocation(
          program,
          "a_texcoord",
        );
  ...
  private getShader(type: GLenum, shaderCode: string) {
    ...
    if (shader) {
      ...
      if (!context.getShaderParameter(shader, context.COMPILE_STATUS)) {
        console.log(
          "Ошибка компиляции шейдера: " + context.getShaderInfoLog(shader),
        );
      ...
      Object.keys(subUniforms).forEach((uniformName) => {
        ...
        if (uniform.type !== "struct") {
          ...
          const setUniformFunction = (
            context[uniform.type as keyof typeof context] as any
          ).bind(context);
