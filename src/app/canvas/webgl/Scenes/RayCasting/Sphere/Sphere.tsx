  ...
  useEffect(() => {
    if (canvasRef.current && pixelatingCanvasRef.current) {
      ...
      const sphereGeometry = new THREE.SphereGeometry(
        uniforms.sphere.data.radius.data,
        32,
        32,
      );
      ...
      const animate = (time: number) => {
        ...
        if (pixelating && material.map) {
          ...
          pixelating.render(
            time,
            (context: WebGL2RenderingContext, program: WebGLProgram) => {
              const spherePosition = uniforms.sphere.data.position.data;
              ...
              const spherePositionUniformLocation = context.getUniformLocation(
                program,
                "sphere.position",
              );
              context.uniform3fv(
                spherePositionUniformLocation,
                uniforms.sphere.data.position.data,
              );
              ...
              context.uniform3fv(
                lightSpherePositionUniformLocation,
                uniforms.lightSphere.data.position.data,
              );
            },
          );
        }
